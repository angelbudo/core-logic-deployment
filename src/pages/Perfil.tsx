        ...
        {loading && !profile ? (
          ...
          <>
            <section className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  <Badge variant="outline" className="gap-1 border-transparent text-white bg-orange-500 ml-auto">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <StatBox icon={<WalletCards className="w-4 h-4" style={{ color: "#93C572" }} />} label="Partides" value={total} accent="font-bold" accentStyle={{ color: "#93C572" }} valueClassName="text-white" />
                <StatBox icon={<ThumbsDown className="w-4 h-4 text-foreground/30" />} label="Abandonades" value={stats?.abandoned ?? 0} accent="text-foreground/30 font-bold" />
                <StatBox icon={<Trophy className="w-4 h-4 text-primary" />} label="Victòries" value={wins} accent="text-primary font-bold" valueClassName="text-white" />
                <StatBox icon={<X className="w-4 h-4" />} label="Derrotes" value={losses} accent="text-destructive font-bold" />
                <StatBox icon={<Star className="w-4 h-4" />} label="% Victòries" value={`${winRate}%`} accent="text-accent font-bold" />
                <StatBox icon={<Flame className="w-4 h-4" />} label="Ratxa màx." value={stats?.max_streak ?? 0} accent="text-orange-500 font-bold" />
              </div>
            ...
            <section className="flex flex-col gap-3 mt-2">
              <div className="text-[10px] font-display tracking-widest uppercase text-primary/85 flex items-center gap-1.5">
                <Users className="w-4 h-4" /> Amics
              </div>
              ...
              <Tabs defaultValue="amics">
                <TabsList className="grid grid-cols-3 w-full">
                  <TabsTrigger value="amics">Amics ({friends.accepted.length})</TabsTrigger>
                  <TabsTrigger value="rebudes">Rebudes ({friends.incoming.length})</TabsTrigger>
                  <TabsTrigger value="enviades">Enviades ({friends.outgoing.length})</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="rebudes" className="avatar-scroll max-h-[45vh] overflow-y-auto pr-2 space-y-2 mt-3">
                  ...
                  {friends.incoming.map((f) => (
                    <div key={f.friendship.id} className="flex items-center justify-between rounded-md border border-primary/25 bg-background/40 p-2">
                      <div className="font-medium text-foreground">{f.other.username ?? "Jugador anònim"}</div>
...
function StatBox({ icon, label, value, accent, accentStyle, valueClassName }: { icon: React.ReactNode; label: string; value: React.ReactNode; accent?: string; accentStyle?: React.CSSProperties; valueClassName?: string }) {
  return (
    <div className="rounded-md border border-primary/25 bg-background/40 p-3 text-center">
      ...
      <div className={`text-xl font-bold ${valueClassName ?? "text-foreground"}`} style={valueClassName ? undefined : accentStyle}>{value}</div>
    </div>
  );
}
