...
import { Link, useNavigate, useParams } from "@/lib/router-shim";
      ...
      <div className="w-full max-w-2xl mx-auto flex flex-col gap-3">
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            <StatBox icon={<WalletCards className="w-4 h-4" style={{ color: "#93C572" }} />} label="Partides" value={total} accent="font-bold text-white" />
            <StatBox icon={<ThumbsDown className="w-4 h-4 text-foreground/30" />} label="Abandonades" value={profile.abandoned} accent="text-foreground/30 font-bold" />
            <StatBox icon={<Trophy className="w-4 h-4 text-primary" />} label="Victòries" value={profile.wins} accent="text-primary font-bold" />
            <StatBox icon={<X className="w-4 h-4" />} label="Derrotes" value={profile.losses} accent="text-destructive font-bold" />
            <StatBox icon={<Star className="w-4 h-4" />} label="% Victòries" value={`${winRate}%`} accent="text-accent font-bold" />
            <StatBox icon={<Flame className="w-4 h-4" />} label="Ratxa màx." value={profile.max_streak} accent="text-orange-500 font-bold" />
          </div>
          ...
          {friendsList.length === 0 ? (
            ...
            <div className="avatar-scroll max-h-[45vh] overflow-y-auto pr-2 space-y-2">
              {friendsList.map((friend) => {
                ...
                return (
                  <Link key={friend.user_id} to={`/perfil/${friend.user_id}`} className="w-full flex items-center gap-3 rounded-md border border-primary/25 bg-background/40 p-2 text-left hover:bg-background/60 transition">
